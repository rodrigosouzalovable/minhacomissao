...
async function gerarBoleto({ cpf, valor_final, tipo_pagamento, parcelas }, cobmais_email, cobmais_senha) {
  ...
  if (currentUrl.includes('Account/Login') || currentUrl.includes('login')) {
    ...
    // Seletores corretos do CobMais: input#Username e input#Password
  ...
  // Se parcelado, preencher número de parcelas
  ...
  // Interceptar PDF antes de salvar
  let boletoUrl = null;
  ...
  // Interceptar novas abas (boleto pode abrir em nova aba)
  ...
  // Se ainda não capturou, tenta clicar em Imprimir
  if (!boletoUrl) {
