...
BEGIN
  ...
  LOOP
    ...
    -- Garante a permissão global de atendimento no Inbox Meta
    IF EXISTS (SELECT 1 FROM public.user_permissions up WHERE up.user_id = r.user_id) THEN
      UPDATE public.user_permissions
      SET atende_inbox_meta = true, atualizado_em = now()
      WHERE user_id = r.user_id AND atende_inbox_meta IS DISTINCT FROM true;
      IF FOUND THEN v_count := v_count + 1; END IF;
    ELSE
      INSERT INTO public.user_permissions (user_id, atende_inbox_meta)
      VALUES (r.user_id, true);
      v_count := v_count + 1;
    END IF;
  END LOOP;
...
-- Correção retroativa: caixa Padrão + todas as caixas criadas
