...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const marco = (confirmadosDepois ?? 0) >= META_WHATSAPP_DIA
      ? "500"
      ...
      : null;
    if (marco) {
      const titulo = marco === "500" ? "Meta diária alcançada" : marco === "400" ? "Captação chegou a 400" : "80% do teto diário consumido";
