...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if ((inst as any).provider === 'uazapi') {
      ...
      // JID real recebido da UAZAPI tem prioridade sobre o telefone reconstruído.
      const destinoUazapi = waJidContato && !/@g\.us|broadcast|@newsletter/i.test(waJidContato) ? waJidContato : to;
