...
BEGIN
  ...
  IF p_chave_idempotencia IS NOT NULL THEN
    SELECT * INTO v_existente FROM public.admin_notificacoes_fila f
    WHERE f.tipo = p_tipo AND f.chave_idempotencia = p_chave_idempotencia AND f.destinatario = p_destinatario LIMIT 1;
    IF FOUND THEN
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.admin_notificacoes_config WHERE id = 1 AND fila_processando_ate > now()) THEN RETURN; END IF;
  SELECT f.id INTO v_id FROM public.admin_notificacoes_fila f
  WHERE f.status = 'pendente' AND f.agendada_para <= now()
  ORDER BY f.agendada_para, f.criada_em FOR UPDATE SKIP LOCKED LIMIT 1;
  IF v_id IS NULL THEN RETURN; END IF;
  UPDATE public.admin_notificacoes_fila SET status = 'processando', tentativas = tentativas + 1 WHERE id = v_id;
  UPDATE public.admin_notificacoes_config SET fila_processando_id = v_id, fila_processando_ate = now() + interval '90 seconds', updated_at = now() WHERE id = 1;
  RETURN QUERY SELECT * FROM public.admin_notificacoes_fila WHERE id = v_id;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.finalizar_notificacao_admin(
  p_id uuid, p_status text, p_instancia_id uuid DEFAULT NULL, p_fallback boolean DEFAULT false, p_erro text DEFAULT NULL
) RETURNS integer
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('admin_notificacoes_processador_global'));
  UPDATE public.admin_notificacoes_fila SET status = p_status, instancia_envio_id = p_instancia_id,
    fallback = p_fallback, erro_detalhe = left(p_erro, 4000), processada_em = now() WHERE id = p_id;
  SELECT f.id INTO v_proxima FROM public.admin_notificacoes_fila f WHERE f.status = 'pendente' ORDER BY f.criada_em LIMIT 1;
  ...
  END IF;
  UPDATE public.admin_notificacoes_config SET fila_processando_id = NULL,
