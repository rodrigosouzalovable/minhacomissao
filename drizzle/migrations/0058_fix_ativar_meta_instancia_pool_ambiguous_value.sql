...
BEGIN
  ...
  IF v_inst.pool_fora_manual IS TRUE THEN
    ...
    SELECT upper(coalesce(string_agg(detail_item #>> '{}', ' '), '')) INTO v_phone_details
    FROM jsonb_array_elements(coalesce(v_inst.saude_restricoes #> '{phone_health,entities}', '[]'::jsonb)) AS health_entity,
         jsonb_array_elements(coalesce(health_entity #> '{additional_info}', '[]'::jsonb)) AS detail_item
    WHERE upper(coalesce(health_entity->>'entity_type', '')) = 'PHONE_NUMBER';
    v_limitacao_apenas_nome := v_phone_details ~ 'DISPLAY NAME|NOME DE EXIBIÇÃO';
    SELECT upper(coalesce(health_entity->>'can_send_message', '')) INTO v_business_can_send
    FROM jsonb_array_elements(coalesce(v_inst.saude_restricoes #> '{phone_health,entities}', '[]'::jsonb)) AS health_entity
    WHERE upper(coalesce(health_entity->>'entity_type', '')) = 'BUSINESS' LIMIT 1;
