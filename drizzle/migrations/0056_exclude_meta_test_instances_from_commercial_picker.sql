...
AS $function$
  ...
  WHERE i.ativo = true
    AND COALESCE(i.instancia_teste_aquecimento, false) = false
    AND public.pode_ver_instancia_meta(auth.uid(), i.id)
  ORDER BY i.nome;
$function$;
REVOKE ALL ON FUNCTION public.get_meta_whatsapp_active_instances_for_sending() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_meta_whatsapp_active_instances_for_sending() TO authenticated, service_role;