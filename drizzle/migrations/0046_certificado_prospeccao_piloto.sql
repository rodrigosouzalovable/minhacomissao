...
CREATE TABLE public.certificado_uazapi_verificadoras (
...
CREATE TABLE public.certificado_prospeccao_envios (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  lead_id uuid NOT NULL REFERENCES public.certificado_leads(id) ON DELETE CASCADE,
...
CREATE INDEX idx_cert_envios_bm_data ON public.certificado_prospeccao_envios (bm_id, reservado_em DESC);
CREATE INDEX idx_cert_envios_status ON public.certificado_prospeccao_envios (status, reservado_em DESC);
...
BEGIN
  ...
  INSERT INTO public.certificado_uazapi_verificadoras(instancia_id,ativa,selecionada_por,updated_at)
...
GRANT EXECUTE ON FUNCTION public.definir_certificado_uazapi_verificadora(uuid,boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.definir_certificado_uazapi_verificadora(uuid,boolean) TO service_role;