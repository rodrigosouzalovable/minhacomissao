...
function montaParcelamento(total: number, descPct: number): string {
  ...
  const partes = opcoes.map((n) => `${n}x de ${moeda(base / n)}`);
...
export function LayoutVistaParcelamentoTab() {
  ...
  async function handleFile(file: File) {
    try {
      ...
      if (out.length === 0)
        throw new Error('Nenhuma linha válida encontrada (A=nome, B=telefone, C=valor)');
      setBase(out);
      setPreview([]);
  ...
  function aplicar() {
    ...
    setPreview(
      base.map((l) => ({
        ...
        parcelamento: montaParcelamento(l.valor, descParcelado),
      })),
    );
