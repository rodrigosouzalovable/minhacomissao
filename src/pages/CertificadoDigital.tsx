...
type Config = { id: string; motor_ativo: boolean; ufs: string[]; cnaes: string[]; janelas_dias: number[]; somente_mei: boolean; somente_celular: boolean; ultima_execucao: string | null; ultimo_status: string | null; total_coletado: number; prospeccao_ativa: boolean; modo_teste_casa_dados: boolean; meta_bm_id: string | null; template_nome: string | null; template_idioma: string; limite_diario: number; prospeccao_pausada_motivo: string | null; prospeccao_instancia_ids: string[] };
...
export default function CertificadoDigital() {
  ...
  const [quantidadeManual, setQuantidadeManual] = useState("50");
  const etapaAtual = indiceExperimentoHoje();
  const porPagina = 25;
  ...
  const { data: preparacaoAtual } = useQuery({
    queryKey: ["certificado-preparacao-atual"],
    queryFn: async () => {
      const { data, error } = await supabase.from("certificado_prospeccao_preparacoes").select("id,quantidade_alvo,status,cnpjs_consultados,leads_novos,numeros_verificados,confirmados_whatsapp,erro,job_id,created_at").order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (error) throw error;
      return data as Preparacao | null;
    },
    refetchInterval: (query) => ["pendente", "processando", "pronta"].includes((query.state.data as Preparacao | null)?.status ?? "") && document.visibilityState === "visible" ? 5_000 : false,
    staleTime: 2_000,
  });
  const { data: metricasExperimento = [] } = useQuery({ queryKey: ["certificado-metricas-experimento"], queryFn: async () => {
    ...
    return JANELAS_EXPERIMENTO.map((janela) => {
      ...
      return {
        ...
        numerosErrados: itens.filter((item) => item.resposta_classificacao === "numero_errado").length,
  ...
  const iniciarManual = useMutation({
    mutationFn: async () => {
      ...
      const instanciaIds = config?.prospeccao_instancia_ids ?? [];
      const { data, error } = await supabase.functions.invoke("certificado-prospeccao-manual", { body: { quantidade, instancia_ids: instanciaIds } });
      if (error) {
        const context = (error as { context?: { json?: () => Promise<{ error?: string }> } }).context;
        const payload = context?.json ? await context.json().catch(() => null) : null;
        throw new Error(payload?.error ?? error.message);
    ...
    onSuccess: () => {
      toast.success("Preparação iniciada. A campanha será criada ao completar a quantidade escolhida.");
      qc.invalidateQueries({ queryKey: ["certificado-preparacao-atual"] });
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível iniciar a preparação"),
  });
  const continuarManual = useMutation({
    mutationFn: async () => {
      if (!preparacaoAtual?.id) throw new Error("Preparação não encontrada");
      const { data, error } = await supabase.functions.invoke("certificado-prospeccao-manual", { body: { action: "continuar", preparacao_id: preparacaoAtual.id } });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      return data;
    },
    onSuccess: () => { toast.success("Preparação retomada"); qc.invalidateQueries({ queryKey: ["certificado-preparacao-atual"] }); },
    onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível continuar"),
  });
  ...
  const alterarInstanciaSelecionada = (instanciaId: string, selecionada: boolean) => {
    const atuais = config?.prospeccao_instancia_ids ?? [];
    salvarConfig.mutate({ prospeccao_instancia_ids: selecionada ? [...new Set([...atuais, instanciaId])] : atuais.filter((id) => id !== instanciaId) });
  };
        ...
        <Card><CardHeader><CardTitle>Histórico de coletas</CardTitle></CardHeader><CardContent><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Quando</TableHead><TableHead>Janela</TableHead><TableHead>Encontrados</TableHead><TableHead>Novos</TableHead><TableHead>Duplicados</TableHead><TableHead>Resultado</TableHead></TableRow></TableHeader><TableBody>{logs.slice(0, 20).map((log) => <TableRow key={log.id}><TableCell>{new Date(log.created_at).toLocaleString("pt-BR")}</TableCell><TableCell>D+{log.janela ?? "—"}</TableCell><TableCell>{log.encontrados}</TableCell><TableCell>{log.novos}</TableCell><TableCell>{log.duplicados}</TableCell><TableCell>{log.erro ? <Badge variant="destructive">Erro</Badge> : <Badge variant="secondary">OK</Badge>}</TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card>
          ...
          <TabsContent value="prospeccao" className="space-y-6">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
               <Card><CardHeader><CardTitle>Template e BM piloto</CardTitle></CardHeader><CardContent className="space-y-4"><div><Label>Template para envio</Label><TemplateFavoriteSelect tipo="meta" className="mt-1" value={templateSelecionado?.id ?? ""} onValueChange={(value) => { const template = templatesHabilitados.find((item) => item.id === value); if (template) salvarConfig.mutate({ template_nome: template.nome, template_idioma: template.idioma, meta_bm_id: null, prospeccao_instancia_ids: [], prospeccao_ativa: false }); }} placeholder="Selecione um template habilitado" options={templatesHabilitados.map((template) => ({ value: template.id, nome: template.nome, idioma: template.idioma, descricao: template.corpo, meta: <Badge variant="secondary" className="text-[10px]">{template.categoria}</Badge> }))} /></div><div><div className="flex items-center justify-between"><Label>BM aprovada</Label>{verificandoBms && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}</div><Select value={config?.meta_bm_id ?? ""} onValueChange={(value) => salvarConfig.mutate({ meta_bm_id: value, prospeccao_instancia_ids: [], prospeccao_ativa: false })} disabled={!templateSelecionado || bmsCompativeis.length === 0}><SelectTrigger className="mt-1"><SelectValue placeholder={!templateSelecionado ? "Selecione primeiro o template" : bmsCompativeis.length === 0 ? "Nenhuma BM com aprovação" : "Escolha uma BM aprovada"} /></SelectTrigger><SelectContent>{bmsCompativeis.map((bm) => <SelectItem key={bm.id} value={bm.id}>{bm.nome} · {bm.quantidade} instância(s)</SelectItem>)}</SelectContent></Select>{templateSelecionado && !verificandoBms && bmsCompativeis.length === 0 && <p className="mt-2 text-xs text-destructive">Este template não está aprovado em nenhuma BM ativa.</p>}{config?.meta_bm_id && (() => { const bm = bmsCompativeis.find((item) => item.id === config.meta_bm_id); return bm ? <p className="mt-2 text-xs text-muted-foreground">{bm.quantidade} instância(s) aprovada(s) · sincronizado {bm.ultimaSincronizacao ? new Date(bm.ultimaSincronizacao).toLocaleString("pt-BR") : "sem data"}</p> : null; })()}</div><div><Label>Instâncias para envio</Label><div className="mt-2 space-y-2">{metaInstancias.filter((inst) => templateStatus.some((item) => item.instancia_id === inst.id && item.status === "approved")).map((inst) => { const apta = inst.estado_pool === "ativo" && !inst.pool_fora_manual && String(inst.saude_status ?? "").toUpperCase() === "CONNECTED"; const marcada = (config?.prospeccao_instancia_ids ?? []).includes(inst.id); return <label key={inst.id} className="flex items-start gap-3 rounded-md border p-3"><Checkbox checked={marcada} disabled={!apta || salvarConfig.isPending} onCheckedChange={(checked) => alterarInstanciaSelecionada(inst.id, checked === true)} /><span className="min-w-0"><span className="block font-medium">{inst.nome}</span><span className="block text-xs text-muted-foreground">{inst.display_phone || "Sem telefone"} · {inst.saude_status || "Sem status"} · {inst.saude_quality || "Qualidade desconhecida"}</span>{!apta && <span className="block text-xs text-destructive">Indisponível para envio</span>}</span></label>; })}{config?.meta_bm_id && metaInstancias.length > 0 && !metaInstancias.some((inst) => templateStatus.some((item) => item.instancia_id === inst.id && item.status === "approved")) && <p className="text-sm text-muted-foreground">Nenhuma instância aprovada para este template.</p>}</div></div><div className="rounded-md border p-3 text-sm"><div className="flex justify-between"><span>Enviados hoje (automático)</span><strong>{enviosHoje} / {config?.limite_diario ?? 50}</strong></div><div className="mt-2 flex justify-between"><span>Instâncias selecionadas</span><strong>{config?.prospeccao_instancia_ids?.length ?? 0}</strong></div></div></CardContent></Card>
                <Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5" />Preparação</CardTitle></CardHeader><CardContent className="space-y-3"><div><Label htmlFor="quantidade-manual">Quantidade de novos contatos com WhatsApp</Label><Input id="quantidade-manual" className="mt-1" type="number" min="1" step="1" inputMode="numeric" value={quantidadeManual} onChange={(event) => setQuantidadeManual(event.target.value)} /></div>{preparacaoAtual && ["pendente", "processando", "pronta", "pausada", "falhou", "campanha_criada"].includes(preparacaoAtual.status) && <div className="space-y-2 rounded-md border p-3"><div className="flex items-center justify-between text-sm"><span>Última preparação</span><Badge variant={preparacaoAtual.status === "campanha_criada" ? "default" : preparacaoAtual.status === "falhou" ? "destructive" : "secondary"}>{preparacaoAtual.status.replace("_", " ")}</Badge></div><Progress value={Math.min(100, (preparacaoAtual.confirmados_whatsapp / preparacaoAtual.quantidade_alvo) * 100)} /><div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground"><span>CNPJs consultados: {preparacaoAtual.cnpjs_consultados}</span><span>Novos: {preparacaoAtual.leads_novos}</span><span>Verificados: {preparacaoAtual.numeros_verificados}</span><span>Com WhatsApp: {preparacaoAtual.confirmados_whatsapp} / {preparacaoAtual.quantidade_alvo}</span></div>{preparacaoAtual.erro && <p className={preparacaoAtual.status === "falhou" ? "text-xs text-destructive" : "text-xs text-muted-foreground"}>{preparacaoAtual.erro}</p>}{preparacaoAtual.status === "pausada" && <Button variant="outline" size="sm" onClick={() => continuarManual.mutate()} disabled={continuarManual.isPending}>{continuarManual.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Continuar preparação</Button>}{preparacaoAtual.job_id && <Button variant="outline" size="sm" onClick={() => ensureJobLoaded(preparacaoAtual.job_id ?? "")}>Ver campanha</Button>}</div>}<Button variant="outline" className="w-full justify-start" onClick={() => verificarWhatsApp.mutate()} disabled={verificarWhatsApp.isPending}>{verificarWhatsApp.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Search className="mr-2 h-4 w-4" />}Verificar números agora</Button><Button variant="outline" className="w-full justify-start" onClick={() => processar.mutate({ simulacao: true })} disabled={processar.isPending}><Activity className="mr-2 h-4 w-4" />Simular próximo envio</Button><div className="flex gap-2"><Input value={telefoneTeste} onChange={(e) => setTelefoneTeste(e.target.value)} placeholder="Telefone para teste" /><Button variant="outline" onClick={() => processar.mutate({ modo_teste: true, telefone_teste: telefoneTeste })} disabled={processar.isPending || telefoneTeste.replace(/\D/g, "").length < 10}><Send className="mr-2 h-4 w-4" />Testar</Button></div><Button className="w-full" onClick={() => iniciarManual.mutate()} disabled={iniciarManual.isPending || ["pendente", "processando", "pronta"].includes(preparacaoAtual?.status ?? "") || Number(quantidadeManual) < 1 || !Number.isSafeInteger(Number(quantidadeManual)) || !(config?.prospeccao_instancia_ids?.length)}>{iniciarManual.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}Iniciar processamento e envios</Button><p className="text-xs text-muted-foreground">Este processamento manual busca CNPJs inéditos e é independente do limite automático diário.</p></CardContent></Card>
            </div>
