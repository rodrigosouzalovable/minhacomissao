...
export default function CertificadoDigital() {
  ...
  const templateSelecionado = templates.find((item) => item.nome === (config?.modo_teste_casa_dados ? "cnpj_atualizado_2" : config?.template_nome) && item.idioma === "pt_BR");
  ...
  const { data: metaInstancias = [] } = useQuery({ queryKey: ["certificado-meta-instancias", config?.meta_bm_id, config?.modo_teste_casa_dados], enabled: config?.modo_teste_casa_dados === true || !!config?.meta_bm_id, queryFn: async () => {
    let query = supabase.from("meta_whatsapp_instances").select("id,nome,display_phone,estado_pool,pool_fora_manual,saude_status,saude_quality,ativo,aquecimento_meta_ativo,meta_bm_id").eq("provider", "meta").eq("ativo", true).eq("instancia_teste_aquecimento", false);
    query = config?.modo_teste_casa_dados ? query.eq("aquecimento_meta_ativo", true) : query.eq("meta_bm_id", config?.meta_bm_id ?? "");
  ...
  const { data: enviosHoje = 0 } = useQuery({ queryKey: ["certificado-envios-hoje", config?.meta_bm_id, config?.modo_teste_casa_dados], enabled: config?.modo_teste_casa_dados === true || !!config?.meta_bm_id, queryFn: async () => {
    ...
    query = config?.modo_teste_casa_dados ? query.eq("template_nome", "cnpj_atualizado_2") : query.eq("bm_id", config?.meta_bm_id ?? "");
        ...
        <Card><CardHeader><CardTitle>Histórico de coletas</CardTitle></CardHeader><CardContent><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Quando</TableHead><TableHead>Janela</TableHead><TableHead>Encontrados</TableHead><TableHead>Novos</TableHead><TableHead>Duplicados</TableHead><TableHead>Resultado</TableHead></TableRow></TableHeader><TableBody>{logs.slice(0, 20).map((log) => <TableRow key={log.id}><TableCell>{new Date(log.created_at).toLocaleString("pt-BR")}</TableCell><TableCell>D+{log.janela ?? "—"}</TableCell><TableCell>{log.encontrados}</TableCell><TableCell>{log.novos}</TableCell><TableCell>{log.duplicados}</TableCell><TableCell>{log.erro ? <Badge variant="destructive">Erro</Badge> : <Badge variant="secondary">OK</Badge>}</TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card>
          ...
          <TabsContent value="prospeccao" className="space-y-6">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
               <Card><CardHeader><CardTitle>Template e números participantes</CardTitle></CardHeader><CardContent className="space-y-4"><div className="rounded-md border p-3"><Label>Template fixo</Label><p className="mt-1 font-medium">cnpj_atualizado_2</p><p className="mt-1 text-xs text-muted-foreground">Olá falo com o(a) responsável por NOME DA EMPRESA? informamos que o processo de emissão do seu CNPJ foi atualizado.</p></div><div><Label>Números de novas BMs com template aprovado</Label><div className="mt-2 space-y-2">{metaInstancias.filter((inst) => templateStatus.some((item) => item.instancia_id === inst.id && item.status === "approved")).map((inst) => { const apta = inst.estado_pool === "ativo" && !inst.pool_fora_manual && String(inst.saude_status ?? "").toUpperCase() === "CONNECTED"; return <div key={inst.id} className="flex items-start justify-between gap-3 rounded-md border p-3"><span className="min-w-0"><span className="block font-medium">{inst.nome}</span><span className="block text-xs text-muted-foreground">{inst.display_phone || "Sem telefone"} · {inst.saude_status || "Sem status"} · {inst.saude_quality || "Qualidade desconhecida"}</span></span><Badge variant={apta ? "default" : "outline"}>{apta ? "Apto" : "Indisponível"}</Badge></div>; })}{metaInstancias.length === 0 && <p className="text-sm text-muted-foreground">Nenhum número está marcado para aquecimento de nova BM.</p>}{metaInstancias.length > 0 && !metaInstancias.some((inst) => templateStatus.some((item) => item.instancia_id === inst.id && item.status === "approved")) && <p className="text-sm text-muted-foreground">O template ainda não está aprovado nos números marcados.</p>}</div></div><div className="rounded-md border p-3 text-sm"><div className="flex justify-between"><span>Enviados hoje</span><strong>{enviosHoje} / {config?.limite_diario ?? 50}</strong></div><div className="mt-2 flex justify-between"><span>Números marcados</span><strong>{metaInstancias.length}</strong></div></div></CardContent></Card>
