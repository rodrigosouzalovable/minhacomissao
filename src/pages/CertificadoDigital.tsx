...
type Preparacao = { id: string; quantidade_alvo: number; status: string; lease_ate: string | null; cnpjs_consultados: number; leads_novos: number; numeros_verificados: number; confirmados_whatsapp: number; erro: string | null; job_id: string | null; created_at: string };
...
export default function CertificadoDigital() {
  ...
  const { data: preparacaoAtual } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("certificado_prospeccao_preparacoes").select("id,quantidade_alvo,status,lease_ate,cnpjs_consultados,leads_novos,numeros_verificados,confirmados_whatsapp,erro,job_id,created_at").order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (error) throw error;
